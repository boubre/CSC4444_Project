...
\begin{document}
    ...
    \frame{ \frametitle{Project Overview}
        \begin{itemize}
            ...
            \item Can we improve classification accuracy of time-series data by
	      adding attributes to current data from previous data?
            \item For example, does informing a classifier of the class of the
    ...
    \frame{ \frametitle{The Naive Bayes Classifier}
        \begin{itemize}
	      ...
	      \(type=sportscar\) to be independent.
            \item Naive Bayes is not sensitive to irrelevant attributes, since the
	      probabilities of such attributes will be similar for all classes.
    ...
    \frame{ \frametitle{SVM Example}
        \begin{columns}[t]
            ...
            \begin{itemize}
                ...
                \item Due to the maximized margin, unseen figures can be closer to
		  the line than the support vectors and still be correctly
		  classified.
    ...
    \frame{ \frametitle{K-Nearest Neighbor}
        \begin{itemize}
            ...
            \item Classification is also simple. To classify point \(x\), find the
	      k points in the training data closest to \(x\). Classify \(x\) as
	      the majority vote its k-nearest neighbors.
            \item Can also weight votes based on the distance of the the neighbors.
            \item KNN suffers from the curse of dimensionality. (In high dimensions
	      points start to become equidistant. This means metrics such as
	      Euclidean distance become unhelpful.)
        \end{itemize}
    }
    ...
    \frame{ \frametitle{Experimental Conjecture}
        \begin{itemize}
	      ...
	      values of \(T[1:i-1]\) may influence the attributes and class
	      value of \(T[i]\).
